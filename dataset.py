...
class Voxceleb(Dataset):
    def __init__(self,
                 folder=os.path.expanduser('/home/server03/voxceleb/result/stit_out_vox1'),
                 batch_size=None,
                 image_size=256,
                 split='train',
                 as_tensor: bool = True,
        ...
        self.split = split
        assert split in ['train', 'test'] and batch_size != None

        self.paths = [p.relative_to(f'{folder}/{split}') for p in Path(f'{folder}/{split}').glob(f'*')]
    ...
    def __getitem__(self, index):
        paths = [p for p in Path(os.path.join(self.folder, self.split, self.paths[index])).glob("*")]  
        batch_paths = random.sample(paths, self.batch_size)
        imgs = []
