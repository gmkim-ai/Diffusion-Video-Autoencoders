...
class GaussianDiffusionBeatGans:
    ...
    def __init__(self, conf: GaussianDiffusionBeatGansConfig):
        ...
        alphas = 1.0 - betas
        self.alphas_cumprod = np.cumprod(alphas, axis=0)   # at
    ...
    def training_losses(self,
        ...
        x_t2 = self.q_sample(x_start, t, noise=noise2)
        ...
        if self.loss_type in [
            ...
            if self.loss_type == LossType.mse:
                if self.model_mean_type == ModelMeanType.eps:
                    # (n, c, h, w) => (n, )
                    terms["mse"] = mean_flat((target - model_output)**2) * self.conf.loss_coef
                else:
                    raise NotImplementedError()
            elif self.loss_type == LossType.l1:
                # (n, c, h, w) => (n, )
                terms["mse"] = mean_flat((target - model_output).abs()) * self.conf.loss_coef
            else:
                raise NotImplementedError()

            with autocast(self.conf.fp16):
                # x_t is static wrt. to the diffusion process
                model_forward2 = model.forward(x=x_t2.detach(),
                                            t=self._scale_timesteps(t),
                                            x_start=x_start.detach(),
                                            **model_kwargs)
            model_output2 = model_forward2.pred

            terms["x0_estimated"] = self._predict_xstart_from_eps(x_t=x_t.detach(), t=t, eps=model_output)
            terms["x0_estimated2"] = self._predict_xstart_from_eps(x_t=x_t2.detach(), t=t, eps=model_output2)
            
            mask = model.encoder.face_mask(x_start.detach())
            terms["x0_estimated"] = terms["x0_estimated"] * mask
            terms["x0_estimated2"] = terms["x0_estimated2"] * mask
            
            if self.loss_type == LossType.mse:
                if self.model_mean_type == ModelMeanType.eps:
                    # (n, c, h, w) => (n, )
                    terms["mse"] += mean_flat((noise2 - model_output2)**2) * self.conf.loss_coef
                    terms["xT_reg"] = mean_flat((terms["x0_estimated"] - terms["x0_estimated2"])**2) * self.conf.xT_reg_coef
                else:
                    raise NotImplementedError()
            elif self.loss_type == LossType.l1:
                # (n, c, h, w) => (n, )
                terms["mse"] += mean_flat((noise2 - model_output2).abs()) * self.conf.loss_coef
                terms["xT_reg"] = mean_flat((terms["x0_estimated"] - terms["x0_estimated2"]).abs()) * self.conf.xT_reg_coef
            else:
                raise NotImplementedError()
        ...
        # del model_forward, decom_model_forward
        return terms
    ...
    def sample(self,
        ...
        if model_kwargs is None:
            ...
            if self.conf.model_type.has_autoenc():
                ...
                model_kwargs['cond_other'] = cond_other
