...
class TimestepBlock(nn.Module):
    ...
    """
    @abstractmethod
    def forward(self, x, emb=None, cond=None, lateral=None, cond_other=None):
        """
    ...
    def forward(self, x, emb=None, cond=None, lateral=None, cond_other=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb=emb, cond=cond, lateral=lateral, cond_other=cond_other)
            else:
                x = layer(x)
        return x
...
class ResBlock(TimestepBlock):
    ...
    def __init__(self, conf: ResBlockConfig):
        ...
        if conf.use_condition:
            ...
            conv_new = conv_nd(conf.dims,
                        conf.out_channels,
                        conf.out_channels,
                        3,
                        padding=1)
            layers = [normalization(conf.out_channels)]
            layers += [
                nn.SiLU(),
                conv_new,
    ...
    def forward(self, x, emb=None, cond=None, lateral=None, cond_other=None):
        ...
        """
        return torch_checkpoint(self._forward, (x, emb, cond, lateral, cond_other),
    ...
    def _forward(
        ...
        if self.conf.use_condition:
            ...
            if self.conf.two_cond:
                ...
                if cond_other is None:
                    cond_other_out = None
                else:
                    cond_other_out = self.cond_other_emb_layers(cond_other).type(h.dtype)

                if cond_other_out is not None:
                    while len(cond_other_out.shape) < len(h.shape):
                        cond_other_out = cond_other_out[..., None]
            else:
                cond_out = None
                cond_other_out = None
...
def apply_conditions(
    ...
    if two_cond:
        ...
        if cond_other is not None:
            while len(cond_other.shape) < len(h.shape):
                cond_other = cond_other[..., None]
            # time first
            # print("blocks.py 314 line check using cond_other for condition")
            scale_shifts = [emb, cond_other, cond]
    else:
    ...
    new_layers, post_layers = post_layers[:3], post_layers[3:]
